...
## 후속 정리 (2026-09-02 · river 지시)

| 항목 | 조치 |
|---|---|
| Filter Chip 요약의 `목록은 Dropdown XSM(34px)` | **`목록은 Dropdown 배포본을 조립`** 로 Select 와 통일 — 요약에서는 자기 크기든 자식 부품 크기든 숫자를 적지 않는다 |
| 개발 코드 위 안내 문구 | 전 화면에서 제거된 상태 확인(9종 × Mobile·PC 실렌더 판독). **렌더 검사 ④로 부활 차단** — 문구를 되살려 보니 9건 발화 후 복구 확인 |
