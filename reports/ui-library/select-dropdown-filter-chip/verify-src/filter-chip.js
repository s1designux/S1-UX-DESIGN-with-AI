...
const NARROW_TRIGGER_THRESHOLD_PX = 140; // dropdown.css min-width:140px 과 동일한 값(정본 미보유, 리터럴).

function syncPanelWidthToTrigger(trigger, dropdownRoot) {
  const triggerWidth = trigger.getBoundingClientRect().width;
  if (triggerWidth > 0 && triggerWidth < NARROW_TRIGGER_THRESHOLD_PX) {
    dropdownRoot.style.minWidth = `${triggerWidth}px`;
    dropdownRoot.style.maxWidth = `${triggerWidth}px`;
  } else {
    dropdownRoot.style.removeProperty("min-width");
    dropdownRoot.style.removeProperty("max-width");
  }
}
...
export function init(root) {
  ...
  const open = () => {
    ...
    syncPanelWidthToTrigger(trigger, dropdownRoot);
