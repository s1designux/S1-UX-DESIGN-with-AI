...
const ids = {
  section: "1562:2",
  screens: ["1562:13", "1562:14", "1562:15"],
...
const pretendardBodyStyles = textStyles.filter(style => {
  ...
  return fontName.family.toLowerCase().includes("pretendard") &&
    fontName.style === "Regular" &&
    style.name.toLowerCase().includes("body");
});
...
function validateModalDual(component) {
  ...
  for (const instanceName of ["secondary", "primary"]) {
...
validateModalDual(nodes.modalDual);
...
const plans = [
  {
    ...
    modalMessage: "에스원은 고객님의 소중한 정보를 보호하기 위해 인증 기기 등록 후 서비스를 제공하고 있습니다.\n현재 인증한 기기로 등록 후 로그인 할까요?",
    modalButtons: ["취소", "확인"]
  },
  {
    screen: screens[1],
...
for (const plan of plans) {
  ...
  await overrideInstanceText(idInput, "텍스트", "s1design");

  const passwordInput = createInstance(nodes.inputFilled, "Input / Password", 320);
  if (passwordPropertyKey) {
    passwordInput.setProperties({ [passwordPropertyKey]: true });
  }
  body.appendChild(passwordInput);
  await overrideInstanceText(passwordInput, "텍스트", "••••••••");
  ...
  if (plan.modalComponent) {
    ...
    const modal = createInstance(plan.modalComponent, "Modal / Device Registration");
