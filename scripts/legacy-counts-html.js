...
<p class="sub">Figma에서 10개 페이지 전부를 다시 읽어 폭 360 화면 ${M.총화면}장의 위치 값을 그대로 쟀습니다. 아래는 <b>바탕 크기(360×780) ${M.바탕780.총화면}장</b> 기준입니다. 반올림하지 않았고, 소수점이 붙은 값은 "어긋남"으로 표시했습니다.</p>
<div class="cards">
...
${M?`<div class="note"><b>간격은 Figma를 다시 읽어 쟀습니다</b> (2-b 참조). 10개 페이지를 모두 읽었습니다.</div>`:`<div class="note"><b>못 잰 것 하나.</b> 판독 원자료에는 부품의 <b>위치·여백 값이 들어 있지 않습니다.</b> 여백 실측이 필요하면 Figma를 다시 읽어야 합니다.</div>`}
...
<h2>6. 정해진 것 (river 결정 2026-09-18)</h2>
<p class="sub">river 지시 — <b>"20으로 통일하고 나머지도 다 정해줘"</b>. 아래대로 규칙 낱장에 적었습니다.</p>
${tbl(['물음','정한 것','근거'],[
 ['화면 뼈대를 몇 벌로?','두 벌 — «틀 있는 화면»(위 틀+본문+아래 틀)과 «덮는 화면»(모달·시트)','레거시 393장 vs 71장'],
 ['위 틀에서 본문까지 간격','20','river 결정 — 레거시엔 기준이 없었음(20·28·16·24 혼재)'],
 ['좌우 여백','20','레거시 실측 왼쪽 252장 / 오른쪽 185장'],
 ['제목줄 자리','정할 것이 아니었음 — 정본 Mobile Header 가 이미 가짐(상태표시줄과 앱바 사이 16)','레거시 40/24 는 옛 자료의 흔들림'],
 ['위·아래 틀 높이','정본 그대로 — 위 Mobile Header(앱 99·웹 149), 아래 Mobile Bottom Nav(탭 1칸 60)','레거시 24/72/48 은 쓰지 않음'],
 ['버튼 종류','한 부품의 크기 축으로 흡수. 텍스트 버튼·보조 버튼만 별개 계열(이미 정해짐)','정본 Button 의 크기 축 · river 2026-09-08'],
 ['입력칸','글 입력과 비밀번호 입력은 한 부품의 축. 고르는 칸은 별개 부품','정본 Input 의 타입 축 · Select 별도'],
 ['화살표','부품 하나 + 회전으로 방향을 만든다','정본이 이미 같은 경로를 −90° 돌려 씀'],
])}
<p class="sub">적은 곳: <code>registry/patterns/rules/screen-skeleton.md</code> · <code>composition.md</code> · <code>registry/governance/composition-rules/R-8.json</code></p>
<p class="sub"><b>아직 안 정한 것</b> — 하단 고정 버튼의 자리·높이, 키보드가 올라왔을 때의 거동, 카드 덩어리. 근거가 없어 «미작성»으로 두었습니다.</p>
