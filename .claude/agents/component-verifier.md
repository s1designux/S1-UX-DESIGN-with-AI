...
> - 정본 V3.0 컴포넌트 = **이 파일 로컬(`remote === false`)**. 허용 출처는 **①로컬(remote=false)** 또는 **②V2.2 아이콘 라이브러리(key `yE5UCFEbmXJBlYJWB24Lz2`·`ic_`)** 뿐.
> - **그 외 모든 `remote === true` = 외부 라이브러리 = ❌(a) FAIL.** 절대 (c)로 흘리지 않는다.
> - ⚠️ **`remote === true`를 "참조된 정본"으로 해석 금지.** ⚠️ **이름이 정본스럽다고(`input`·`button`·`ic_*`) 로컬로 가정 금지** — 외부 라이브러리도 같은 이름을 쓴다.
