...
4-1. **레거시 이름은 눈으로 맞추지 않는다 — 기계에 묻는다.** 레거시(A·B) 세트·상태 이름이 지금 정본의 무엇인지는 `npm run legacy:resolve -- "<레거시 이름>" [--state x] [--size y]` 로 조회한다(전체 훑기 `-- --all`). 답은 붙음·배치 규칙·레거시에만 있음·정본에 대응 없음·결정 전 다섯 가지이며 **근거 결정 번호가 함께 나온다.** `결정 전`·`모름`이면 **스스로 이름을 정하지 말고 (c)로 올린다.** 조회기가 틀렸다고 보이면 river 결정 원본(`reports/legacy-crosswalk-board/crosswalk.json`)이 기준이고, 표를 손으로 고치지 않는다(Gate 52 가 대조한다).
