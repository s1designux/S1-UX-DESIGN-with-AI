...
4-1. **레거시 이름은 눈으로 맞추지 않는다 — 기계에 묻는다.** 레거시(A·B) 세트·상태 이름이 지금 정본의 무엇인지는 `npm run legacy:resolve -- "<A|B>:<레거시 이름>" [--state x] [--size y]` 로 조회한다(전체 훑기 `-- --all`). **반드시 `A:`/`B:` 접두사를 붙인다** — 같은 이름이 두 파일에 다 있으면 접두사 없이 물었을 때 답이 갈린다(조회기는 그때 "이름이 겹침"으로 되묻는다). 답은 붙음 · **세트는 붙음 · 이 축은 결정 전(partial)** · 배치 규칙 · 레거시에만 있음 · 결정 전 · 모름이며 **근거 결정 번호가 함께 나온다.** **축을 물었는데 `axes` 가 비어 오면 답이 아니라 오류로 본다** — 그때는 `unmapped` 에 이유가 적혀 있고, 그 값은 (c)로 올린다. `결정 전`·`모름`은 **스스로 정하지 말고 (c)로 올린다.** 자동 훑기가 짝을 못 찾은 것도 «대응 없음» 이 아니라 `결정 전` 으로 나온다 — 기계가 «없다» 고 단정하지 않는다(river 결정 2026-09-17). 조회기가 틀렸다고 보이면 river 결정 원본(`reports/legacy-crosswalk-board/crosswalk.json`)이 기준이고, 표를 손으로 고치지 않는다(Gate 52 가 대조한다).
