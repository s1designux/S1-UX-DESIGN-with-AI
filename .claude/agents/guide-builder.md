...
## figma-to-code 워크플로우 내 역할 — 구현 전담 (3·5단계)

> 상세 워크플로우: `.claude/skills/figma-to-code/SKILL.md`

`figma-to-code` 5단계 워크플로우에서 이 에이전트는 **구현자(Builder)** 다.

- **3단계 구현** — 1단계 재고조사 목록의 variant를 하나도 빠짐없이 구현한다. **2단계 수치추출표의 값만** 사용하고, 원본 구조를 그대로 따른다(새로 디자인 금지). 아이콘은 MCP 원본 에셋을 그대로 사용한다.
- **5단계 다크모드 구현** — navy 5단계 표면 위계 적용, 팝업·드롭다운은 다크에서도 라이트 유지, `[data-theme="dark"]` CSS 선택자만 사용(HTML forced-dark 금지).

> **4단계 자가대조는 이 에이전트가 하지 않는다.** 구현 완료 후 별도 검증 전용 에이전트
> `component-verifier`에게 넘긴다(만드는 자 ≠ 검증하는 자). `component-verifier`가 ❌ 목록을
> 돌려주면 3단계로 되돌아가 수정한다. ❌가 0이 될 때까지 반복한다.
>
> Harness Audit(Part 2, `npm run harness:audit`)은 **Gate 5 사후 검문**으로 계속 유효하며,
> figma-to-code 4단계에서는 `component-verifier`가 이 스크립트를 대조 도구로 사용한다.
