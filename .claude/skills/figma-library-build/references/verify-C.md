...
> **① 기계검사 선행:** 아래 결정론 스캔(토큰 바인딩·폰트·raw hex)을 🏗️ 빌더/⭐ 가 **빌드 종료 전에 먼저 돌려 위반 0** 으로 만들고 결과 표를 넘긴다. 위반이 남은 채 검증자를 부르지 않는다(반려 왕복 제거). ⚠️ **이 스캔들은 하드룰상 검증자가 다시 실행해야 하는 항목** — 선행 실행이 재실행을 면제하지 않는다. 줄어드는 건 왕복이지 검사가 아니다.
> **② 렌더 선캡처:** variant `get_screenshot` 을 미리 떠서 **파일 경로 + 캡처 조건**으로 넘긴다. 검증자는 판정이 갈리는 지점에서만 다시 뜬다.
> **③ 델타 재검증:** 재검증이면 직전 `4-verification.md` + 변경된 노드 목록·지문을 넘긴다. 승계 항목은 "이번에 재확인하지 않음"으로 표기한다(승계 금지 3조건은 공통 계약 ②).
