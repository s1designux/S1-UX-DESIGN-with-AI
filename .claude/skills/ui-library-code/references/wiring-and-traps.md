...
PC·Mobile 두 섹션에 같은 문자열을 넣으면 **문서 안 모든 `id`** 가 중복된다 — 라디오 `name`·`id` 만이 아니다.

두 갈래로 터진다:

1. **라디오** — 같은 name 의 라디오는 하나만 선택될 수 있어 먼저 그려진 쪽 선택이 사라진다.
2. **SVG 내부 id**(`mask`·`filter`·`clipPath`·`linearGradient`) — `url(#...)` 은 문서 순서상 **첫 번째**를 가리킨다.
   그 첫 번째가 **숨겨진(`display:none`) 블록 안**이면 마스크·필터가 아예 적용되지 않아 **그림이 뭉개진다.**
   2026-09-02 F-5: 목업 wifi 가 호 2겹 대신 속 찬 부채꼴로 배포됐다. 라디오는 정확히 방어했는데 SVG 는 놓쳤다.

- 자리마다 새로 생성한다. **같은 문자열을 두 블록에 재사용하지 말 것** — 생성 함수를 두 번 부른다.
- SVG 내부 id 는 인스턴스마다 고유 접미사를 붙인다(카운터).
- 렌더 후 확인: **페이지 전체** 중복 `id` 0개(라디오만 세지 말 것). 마스크를 쓰는 그림은 **모양까지** 본다 — 수치·DOM 검사는 전부 통과하면서 그림만 틀릴 수 있다.
