...
> 공통 규칙 = `.claude/agents/component-verifier.md` §검증 입력 계약.
> **① 기계검사 선행:** `npm run harness:audit` · `npm run gate:check` 를 ⭐가 먼저 돌려 ❌ 0 으로 만들고 명령·종료코드 표를 함께 넘긴다. 표가 없거나 실패가 남았으면 검증자는 **HOLD("기계검사 선행 미이행")** 로 즉시 반환한다. 검증자는 그 명령을 1회 재실행해 종료코드만 확인하고, 통과한 항목을 눈으로 다시 대조하지 않는다.
> **② 렌더 선캡처:** Figma `get_screenshot` 원본과 구현 렌더 캡처를 미리 떠서 **파일 경로 + 캡처 조건(노드 id·URL·뷰포트·테마·시각)** 으로 넘긴다. 검증자는 판정이 갈리는 지점에서만 다시 띄운다 — 단, 아래 §시각·레이아웃 대조의 **실측(`getBoundingClientRect`·글리프 비율)은 여전히 직접** 한다.
> **③ 델타 재검증:** 재검증이면 직전 `4-verification.md` + 그 이후 변경된 파일 목록·지문을 넘긴다. 승계한 항목은 "이번에 재확인하지 않음"으로 표기한다(승계 금지 3조건은 공통 계약 ②).
