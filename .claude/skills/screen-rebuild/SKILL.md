...
**레거시 화면 이름을 그대로 복사하지 않는다.** 값·구조는 정본으로 옮기면서 이름만 원본에서 복사하면, 없어진 기획서 목차 번호가 산출물에 남아 이름으로는 화면 맥락을 알 수 없게 된다(2026-08-25 모바일 로그인 11개 전량이 이 상태였다).

각 화면 프레임의 **새 이름을 매핑표에 함께 적는다.** 서식·흐름 코드 규칙의 기계 정본은 `registry/governance/screen-naming-policy.json` 이다 — `{PLATFORM}/{PATTERN}/{FLOWCODE} · {한글 상태명}{ (표현)}`, 숫자만=기본 흐름 · `4a1`=4단계에서 갈라진 첫 분기의 1번째. 캔버스 배치와 Section 자식 순서도 그 코드 순서에 맞춘다.
...
**spawn 전 준비 (2026-09-04 river 승인 — 공통 규칙 = `.claude/agents/component-verifier.md` §검증 입력 계약):**
- **기계검사 선행** — 빌드 종료 전에 `provenance-scan` · `figma-font-scan` · raw hex 스캔을 **빌더/⭐가 먼저 돌려 위반 0** 으로 만들고, 그 결과 표를 검증 요청문에 넣는다. 위반이 남은 채 검증자를 부르지 않는다(반려 왕복 제거). ⚠️ **단, 이 스캔들은 하드룰상 검증자가 다시 실행해야 하는 항목이다** — 선행 실행은 검증자의 재실행을 면제하지 않는다. 줄어드는 건 왕복이지 검사가 아니다.
- **렌더 선캡처** — 원본 `get_screenshot` 과 재현 화면 캡처를 미리 떠서 **파일 경로 + 캡처 조건**으로 넘긴다. 검증자는 판정이 갈리는 지점에서만 다시 뜬다.
- **델타 재검증** — 재검증이면 직전 보고서 + `snapshot-before/after` + `snapdiff` 결과를 넘긴다. 검증자는 직전 ❌ 항목과 변경이 닿은 범위만 보고, 나머지는 "직전 PASS 승계"로 **명시**한다(승계 금지 3조건은 공통 계약 ②).
...
- 원본 요소 **누락 0건**
- **오버레이에 가려진 배경 = 가려지지 않은 기준 화면과 일치** — 팝업·모달·바텀시트가 덮은 화면의 **배경(오버레이 노드를 뺀 나머지)** 은, 같은 상태를 오버레이 없이 보여주는 기준 화면과 **전수 일치**해야 한다. 눈으로 안 보이니 검증도 빠지고, 그 결과 레거시 시절 문구·상태가 가려진 채 남는다(2026-08-25: 팝업 4개 화면의 보조 버튼만 레거시 문구 `휴대폰 번호로 로그인` 으로 남아 있었다 — 제작 후 독립 검증을 두 차례 거치고도 안 걸렸고 사용자가 발견했다).
  **대조 방법:** 오버레이 노드를 제외하고 경로 기반으로 평탄화해 기준 화면과 필드 단위 비교(이름·문구·크기·좌표·채움·텍스트스타일·폰트). 차이는 ❌(a) 로 처리하고 **기준 화면 쪽으로** 맞춘다.
