---
name: screen-rebuild
description: "Figma에서 화면/플로우/패턴을 만들거나 고치는 모든 작업의 단계형 검문소 워크플로우. (1) 레거시 시안을 최신 정본(V3.0) 컴포넌트로 '원본과 동일하게' 재현할 때, (2) **이미 만들어 둔 패턴·화면을 수정·보완할 때**('이 화면에 ~ 추가해줘', '~가 빠졌어', '구분선/문구/상태 고쳐줘'), (3) **레거시 원본 없이 새로 제작할 때**, (4) 이전 결과 재실행·업데이트, 시간·토큰을 줄인 안전한 재구성 요청에 반드시 사용한다. 착수 시 기준(baseline)을 legacy/existing-nodes/intent-spec 중에 선언하고, 기존 노드를 고치면 변경 전 구조 스냅샷을 먼저 뜬다. 대표 상태 선검증 뒤 일괄 생성하고 전체 결정론 검사를 유지해 품질을 낮추지 않는다. figma-to-code와 반대 방향: 의도/레거시 → V3.0 Figma."
---
...
├── intent.md              # 의도 선언서 (baseline=intent-spec 일 때 필수 · 사람 승인)
...
## 0단계 — 기준(baseline) 선언 · 착수 전 필수

**"무엇과 대조해서 맞다고 판정할 것인가"를 먼저 정한다.** 이게 없으면 검증자가 대조할 것이 없다.

축은 두 개고 서로 독립이다: ①**대상 노드가 이미 있나**(→ 스냅샷 diff 필요) ②**레거시 원본이 있나**(→ 정답의 기준).

| 선언 | 언제 | 필수 산출물 | 검증자가 대조하는 것 |
|---|---|---|---|
| `legacy` | 레거시 원본을 재현 | `1-inventory.md` (원본 전수) | 원본 ↔ 결과 |
| `existing-nodes` | 이미 있는 노드를 수정 | `snapshot-before/expect/after.json` + snapdiff 통과 기록 | diff ↔ 기대 선언 |
| `intent-spec` | **레거시 없이 신규 제작** | **`intent.md`(의도 선언서) + 사람 승인** | 선언서 ↔ 결과 |

겹치면 여러 개를 동시에 선언한다(예: 레거시 재현인데 기존 노드도 고침 → `["legacy","existing-nodes"]`).

`workflow-state.json` 에 기록하고, **Gate 40** 이 선언과 산출물의 정합을 검사한다(현재 경고 단계).

```json
"evidence": {
  "baseline": ["existing-nodes"],
  "snapdiff": { "ranAt": "2026-08-24", "violations": 0, "by": "component-verifier" },
  "intent":   { "doc": "intent.md", "approvedBy": "river", "approvedAt": "2026-08-24" }
}
```

> 규약 신설(2026-08-24) 이전 작업은 `evidence.exempt.reason` 으로 동결한다. 신규만 검사한다(래칫).

### 🚨 `intent-spec` — 레거시가 없을 때

**레거시가 없다는 것은 "자유롭게 만들어도 된다"가 아니라 "기준을 사람이 줘야 한다"는 뜻이다.** 근거 없이 빈자리를 메우는 것이 하드룰 H6② 가 막는 실패이고, 이 스킬이 태어난 원인(원본을 안 읽고 지어냄)이다.

**의도 선언서(`intent.md`)가 없으면 착수하지 않는다.** 레거시가 하던 "정답지" 역할을 사람이 미리 쓴 선언서가 대신한다.

선언서에 **필요한 것** — 레거시에서 읽던 것과 같은 항목이다:
- 화면 목록과 각 화면의 상태(무엇을 보여주는 순간인가)
- 각 화면의 문구 (verbatim)
- 쓸 정본 컴포넌트와 그 상태

선언서에 **필요 없는 것** — 이미 정답이 있는 것은 선언하지 않는다:
- 컴포넌트 **규격**(크기·색·상태 스타일) → `build-components.ts` 정본이 정답
- 같은 종류 패턴의 **규칙** → `registry/patterns/*/content-rules.md` 가 정답
- 토큰 값·타이포 → 정본이 정답

선언서로도 안 정해지는 것이 나오면 지어내지 말고 `needs-decision` 으로 올린다.
...
> `baseline` 이 `intent-spec` 뿐이면(레거시 원본 없음) 이 단계는 **건너뛴다.** 대신 0단계의 의도 선언서가 재고조사 결과를 대신하며, 4단계 대조도 원본이 아니라 선언서를 기준으로 한다.
