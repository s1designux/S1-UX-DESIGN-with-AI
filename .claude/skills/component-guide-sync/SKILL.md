...
description: "컴포넌트 정본(build-components.ts)의 크기·variant·상태·토큰·레이아웃 변경을 guide model과 설치기에 자동 반영한다. 메인 사이트 pages/components.html은 손관리 화면이며 자동 생성하거나 덮어쓰지 않는다."
---

# Component Guide Sync

컴포넌트의 시각 값은 `plugins/figma-vars-installer/src/build-components.ts` 한 곳에서 관리하고,
guide model과 설치기는 결정론적 생성 결과를 소비하게 하는 오케스트레이션 스킬이다.
메인 사이트는 기존 손관리 화면을 보존하며, 별도 사이트 변경 요청 없이는 수정하지 않는다.
...
| 구조 변경 검증 | `component-verifier` | 정본→guide model→설치기 대조, 사이트 무변경 확인 |
...
`pages/components.html`은 이 명령 전후 파일 지문이 같아야 한다.
...
- `tokens:reconcile` 전후 `pages/components.html`이 바뀌지 않았는지 확인한다.
...
사이트를 별도 요청으로 변경한 경우에만 Light/Dark와 PC/Mobile 실제 렌더를 확인한다. 렌더 도구 부재를 PASS로 처리하지 않는다.
...
- reconcile 뒤 사이트가 바뀜: 즉시 실패 처리하고 사이트 생성 배선을 제거한다.
...
"Line Tab에 새로운 size를 추가해줘" → 정본 한 곳 수정 → reconcile → guide model·설치기 자동 갱신 → 사이트 파일 무변경과 Gate 통과.

### 오류

guide model에서 새 size 한 줄을 손으로 삭제 → `components:guide-model:check` 실패 → 정본/생성기에서 복구하기 전 완료 금지.
