...
- **역할 기반 semantic**(`--color-bg/surface/text/border/action/status`) = `assets/css/site-base.css` (페이지가 `style.css → @import site-base.css`로 로드하는 라이브 파일). **단, `color/bg/*`·`color/surface/*`는 2026-06-16부터 vars-data SEMANTIC_COLOR에도 편입**(시멘틱 토큰으로 Figma·설치기 노출). 이 둘은 vars-data가 우선 정본이고 site-base.css는 동일 값 포털 폴백으로 공존. 나머지 역할토큰(text/border/icon/action/status 등)은 여전히 site-base.css 전용(vars-data엔 없음).
