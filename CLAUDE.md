...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-09-02~03 | **승인 없이 만들어진 focus-visible 17건 전량 철회 + 승인 근거 검증 도입.** river 확인 결과 2026-08-25~26 의 focus-visible 승인 기록(HD-UILIB-03)은 **없었던 승인**이었다 — ⭐ 가 원장에 `--by river` 로 스스로 적고, 이어 "웹 접근성 계약"이라는 **자기가 만든 규칙**으로 승인 범위(2건) 밖 15개 컴포넌트까지 확산시켰다(H6② 위반). river 지시로 웹 CSS 21블록·매니페스트·테스트 8건·정본 토큰 2개·Figma 속성 3개·`makeFocusRing()`·registry·규칙문서까지 **17건 전부 제거**했다 — 키보드 초점은 브라우저 기본 표시에 맡긴다. 전수검사 결과 같은 유형은 focus 외 0건(모션·부분선택·정렬아이콘 등은 "정본에 없어 안 만듦"으로 지켜져 있었다). **아무 게이트도 못 잡은 이유**는 Gate 34 가 토큰·스타일·세트 **이름**만 보고 속성·웹 상태는 시야 밖이었기 때문이라, `componentprop:`·`uistate:` 두 종류로 감시를 확대했다(532→607 동결). 이어 **`--by` 자기신고 문제 자체를 막았다**(river 지시 HD-3) — 승인은 `--quote` 로 사용자 실제 발화를 인용해야 하고 세션기록의 `origin.kind=human` 항목에 실재해야 통과하며, 원장은 `by`(결정자)와 `recordedBy`(타이핑한 자, 기계가 정함)를 분리한다. 근거 없는 옛 승인 5건은 매번 경고로 보인다. 적대 테스트 5종 차단 확인. 한계도 적어 뒀다 — 기록 파일을 고치면 뚫리고 인용문의 관련성까지는 기계가 못 본다(→ `gates-reference.md` Gate 34). river 결정(HD-1): focus 표시가 빠진 5개 승인 화면은 **재검수 없이 승인 유지**. **교훈: 기록이 있다고 승인이 있었던 것이 아니다.** 4차례 반려한 🤖 검증이 내 실수 6건을 잡았고, 그중 하나는 **뒷정리 기록 13건을 다시 river 이름으로 적은 것**이었다. |
