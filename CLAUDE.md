...
### Gate 7: Token Value Consistency Gate (2026-06-11 신설)
**파일:** `scripts/token-value-consistency-check.js`
**트리거:** `assets/css/tokens.css`·`plugins/figma-vars-installer/src/vars-data.ts`·`pages/semantic.html` 변경 / 항상
**자동 스크립트:** `npm run tokens:consistency` 또는 `npm run gate:check`
**역할:** 토큰 "값"이 표면 간 *실제로 일치*하는지 기계 판정. `token-sync` 에이전트(작업자)가 전파를 빠뜨려도 이 Gate(검사기)가 잡는다. **값 동기화의 최종 검증 주체는 에이전트가 아니라 이 Gate다.**
**체크 항목:**
- **Check A (ERROR):** tokens.css ↔ vars-data.ts — 두 파일에 동일 이름으로 존재하는 semantic 토큰(교집합)을 각각 Foundation HEX까지 해석(tokens.css는 semantic→foundation 체인 추적, vars-data는 FOUNDATION_COLOR HEX)해 Light/Dark 모두 대조. 불일치 = 설치기 zip이 가이드와 다른 색을 심음.
- **Check B (ERROR):** pages/semantic.html 표의 하드코딩 light/dark HEX ↔ tokens.css 해석값. 불일치 = 문서 표 stale.
- 이름이 한쪽에만 있는 토큰은 비교 제외(naming 차이 false positive 방지). 값 불일치만 FAIL.
**한계:** 현재 vars-data 값 일치(A)·semantic.html hex(B)만 강제. semantic.md/component-tokens-extracted.md 텍스트 정합은 미커버 — token-sync 에이전트가 챙김.
**도입 사유:** 2026-06-11 control-bg-disabled 값 정정 시, vars-data 값 드리프트·semantic.html stale hex를 기계가 못 잡는 사각지대 확인. 사용자 요청으로 값 일치 검사를 Gate로 승격. 도입 시 semantic.html stale hex 10건(form-control dark 6·data-state-hover·text-state-helper·correct) 발견·정정.
...
작업 완료 직전:
  ...
  7. Gate 7 (Token Value Consistency) — 토큰 값(tokens.css·vars-data·semantic.html) 변경 시 / 항상
```

스크립트 일괄 실행: `npm run gate:check` (Gate 1 + 3 + 4 + 6 + 7 자동)
