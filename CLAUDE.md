...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-09-14 | **안드로이드 부품 10 → 16종 + 하단 내비에 icon 축 신설(river 승인).** 모바일 헤더·하단 내비 → 여러 줄 입력·글자/보조 버튼·필터 칩 순으로 옮겼고(모달 속틀은 정본에 모바일 크기가 없어 제외), 하단 내비는 칸마다 아이콘이 달라야 바를 조립할 수 있어 **icon(home·search·notification·settings) × state = 8변형**으로 늘렸다. 새 아이콘 3종은 Figma V2.2 Solid 원본을 `node.key` 로 직접 확인해 등록(원본 픽셀 대조 통과). 🤖 component-verifier 재검증 후 Gate 13 기록. 값은 브라우저 실측 542건 대조 PASS. 같은 날 **배포본 번호를 기계로 넘기고 Gate 50 신설** — 0.1.0 → **0.4.0**. |
