...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-09-02 | **안내 화면 '부품 표본 격리' 규칙 신설 + Gate 44 배선(46→47개).** Table 셀 표본을 컴포넌트 루트로 감싸는 바람에 표 외곽선(위 2px·아래 1px)이 낱개 셀마다 그려져 **정본에 없는 모습**이 안내 화면에 나갔다(river 지적). 배포본 CSS 는 정확했고 표본 조립만 틀려서 계약·파리티·수치 대조 검사기가 전부 통과한 사각지대였다. 규칙(부품 표본은 부품이 소유한 것만 보여준다)을 `component-presentation-policy.json` 에 선언하고, 렌더된 DOM 과 실제 dist CSS 를 대조하는 `ui-guide-part-sample-check.js` 로 집행한다(적대 테스트 포함·실제 회귀 재현으로 검증). 같은 김에 여태 gate:check 밖에 있던 안내 화면 렌더 검사 전체를 **Gate 44** 로 커밋 검문소에 넣었다. |
