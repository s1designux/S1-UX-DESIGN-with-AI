...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-08-03 | **정본 단일화 1~4단계 — 「정본 목록의 정본」(canon-manifest + Gate 36)·문서 모순 제거·Gate 34 컴포넌트 확장·환경 의존 봉합** — river 질문("여러 벌로 안 나뉘고 정본 하나에서 관리하고 싶다")에 대해, 정본 3벌은 Figma 구조상 병합 불가(Variables·Text Styles·Components = 다른 개체·다른 설치 API)임을 실측 확인하고 **파일 병합 대신 선언·집행을 한 곳으로 수렴**. ①`registry/governance/canon-manifest.json` 신설 — 정본 3벌·파생 13표면·재생성 명령·담당 게이트·정책 정본 21·메타 장부 4를 기계가독 선언(registry/components 는 "메타의 정본이지만 값의 정본은 아니다"를 isCanonFor/isNotCanonFor 로 분리). ②**Gate 36** 신설 — 선언↔실제 배선 양방향 대조(우산이 도는데 미선언=차단 · 유령 명령·정본 몰래강등=차단), 적대 4종 확인. 대시보드는 휴리스틱을 유지하되 선언과 **대조**해 `선언 정본/정책 정본/메타 장부/선언상 파생/미분류` 표시 → 12파일 전부 분류·미분류 0(종전 휴리스틱은 스크립트 2개를 정본 오분류·자기선언 파생을 정본 분류). ③**문서 모순 제거** — 옛 MVP0 "충돌 시 registry JSON 우선"이 하드룰 H6 과 정면충돌하던 것을 5곳(CLAUDE.md 2·gen-design-md·gate-check·design.manifest) 정리, **AI 프롬프트 헤더의 잘못된 정본 안내**도 교정. ④**Gate 34 확장** — H7 이 컴포넌트 신설도 금지하는데 게이트는 토큰·스타일만 봤다(Gate 30 은 커버리지지 승인 아님) → `component:<이름>` 42개 추적(486→528), `--extend-tracking`(1회성·종류 제한) 신설, 적대 4종 중 **재확장 우회로를 테스트로 발견해 봉합**. ⑤**환경 의존 3건 봉합** — 저장소를 Windows 로 옮긴 뒤 gate:check 가 18 error 로 커밋을 막고 있었고 전부 판정이 아니라 환경 문제였다: `.gitattributes`(LF 고정 — CRLF 가 12건 거짓 드리프트 유발) · `read-zip-entry.js`(unzip 부재) · `extract-tar.js`(`/bin/sh` 부재 + 셸별 tar 차이). 부수로 루트 색인 누락 9개(웹 번들에서도 빠져 있었고 감시 게이트 0)를 등재하고 Gate 30 에 (2b) 대조 추가. gate:check PASSED(39개). |
| 2026-07-30 | **설치기 UI 0.2Ver — 앱셸 레이아웃 + 3화면 상태기계** — 버튼영역 관통 스크롤 제거(문서 스크롤러 폐지·`.iscroll` 단일 스크롤·푸터 고정·창높이 720→800), 표시명 `S-1 S/W UX Guide 설치/검수기`+손버전 배지(ui.html·code.ts·manifest·install-prompt 전파), 설치 중 전용 화면(스피너·퍼센트 단조클램프·화면이동 금지 경고·탭 잠금), 완료/오류 전용 화면 + `[재설치]`(확인 카드 → 현재 페이지 카테고리 SECTION 삭제 → 첫 화면; Variables·TextStyles 보존), 오류 화면 빈 패널 버그(존재하지 않는 `#legacy-progress-wrap` TypeError) 수정 + 죽은 레거시 스왑 JS 140줄 제거. 440×800 렌더 프로브로 스크롤 0·푸터 가시성 검증. 진행률 예산 재배분은 후속(river 결정). |
| 2026-07-14 | **GUI 검수기(레거시검수) 설치기 통합** — 별도 `figma-component-audit` 플러그인의 색상검수(HEX·external-var 탐지→토큰 제안→적용)+컴포넌트교체 엔진을 설치기(`figma-vars-installer`)로 이식해 **[설치][검수] 한 플러그인**으로 통합(river 결정). 신규 `audit-engine.ts`(엔진 이식·export)·`audit-report.ts`(검수 결과서 캔버스 프레임 4섹션). code.ts 라우터는 `audit:` 접두사로 설치/검수 메시지 격리(옛 레거시 스왑 함수는 죽은코드로 보존), ui.html은 `#audit-root` 스코프로 CSS 절연 + [검수] 탭에 색상검수/컴포넌트교체/**가이드 외 색상**(classifyOffGuide·Δ12)/**리포트** 서브탭. showUI 창크기는 B값(440·720) 유지. tsc 통과. [[three-checkers-and-gui-audit-merge]] |

> 위 이전(2026-04-29 ~ 2026-07-10)의 전체 상세 이력 78건: **`reports/changelog-archive.md`** 참조.
...
- 🚫 **하드룰 — 「정본 우선」: 파생 표면의 값을 판정 근거로 쓰지 않는다(2026-08-03 신설).** **정본**(`plugins/figma-vars-installer/src/vars-data.ts` · `textstyles-data.ts` · `build-components.ts` — **기계가독 정본 목록 = `registry/governance/canon-manifest.json`, Gate 36 이 선언↔배선 대조**)과 **파생 표면**(웹 가이드 `pages/*.html` · `registry/**` · 컴포넌트 미리보기 · `design/DESIGN*.md` · `reports/**`)이 다르면 **저울질 없이 파생을 고친다.** 파생 값은 **증거가 아니다** — 대개 프로토타입 잔재이거나 손으로 옮겨 적다 어긋난 것이다. **따라서 "파생이 A 인데 정본은 B 다, 어느 쪽이 맞나?"는 성립하지 않는 질문이며, 그 형태로 사용자에게 묻는 것 자체가 규칙 위반이다.** 성립하는 질문은 **"정본을 A 로 바꿀까요?"** 하나뿐이다. 완료 보고에서도 파생의 변경을 *비용·부작용*처럼 서술하지 않는다 — 그건 원래 그래야 했던 상태로 돌아가는 것이다. **적용 범위 주의:** §🚫 「두 갈래 분류」는 **레거시(DS 2.4) ↔ 정본** 사이에만 적용한다. **정본 ↔ 파생 사이에는 적용 금지** — 파생이 정본과 다른 것은 (b)'사전 등록된 개선'이 될 수 없다. (근거: 2026-08-03 세션에서 ⭐가 "웹 24 vs 정본 16 중 어느 쪽?"·"탭 굵기 Bold/Medium 중 어느 쪽?"을 반복해 묻고, "웹이 20px 이라 바뀝니다"를 비용처럼 서술해 사용자가 세 번 교정. 원인은 문서에 정본↔파생 판정 우선순위가 없었고 두 갈래 분류가 넓게 오독될 여지가 있었던 것.)
...
| 훅 | 위치 | 발동 | 동작 | 강제력 |
|----|------|------|------|--------|
| **PreToolUse** (차단) | **`.claude/settings.json`(프로젝트·커밋됨)** → `scripts/figma-code-hex-check.js` | `use_figma` 호출 직전 | code 안 하드코딩 hex(`#RRGGBB`/`#RGB`) 탐지. 있으면 exit 2로 **도구 실행 차단**(Figma 노드 색=Variable 바인딩 강제). 순수 장식 크롬만 `// figma-hex-allow: 사유` 예외 | 물리 차단 |
| **PreToolUse** (차단) | **`.claude/settings.json`(프로젝트·커밋됨)** → `scripts/figma-code-font-check.js` | `use_figma` 호출 직전 | code 안 비-Pretendard 폰트(`fontName`/`loadFontAsync` family) 탐지. 있으면 exit 2로 **차단**(캔버스 텍스트=Pretendard 정본 강제). 일시 Noto 는 줄 끝 `// figma-font-temp: <재바인딩 계획>` 마커 필수(인지 강제, 최종 검증은 L2 figma-font-scan). 정본 `registry/governance/figma-font-policy.json` | 물리 차단 |
...
- **재설치:** git 훅은 커밋되지 않지만, **package.json `prepare` 스크립트가 `npm install` 시 자동 설치**하므로 보통 수동 입력 불필요. (수동 강제: `bash scripts/hooks/install-git-hooks.sh`. git 없는 환경에선 조용히 통과. ⚠️ Windows 에서는 `npm install` 의 prepare 단계가 `bash` 를 못 찾아 실패할 수 있다 — 그때만 Git Bash 에서 위 명령을 1회 실행.)
- **PreToolUse 훅 이식성 (2026-08-03 봉합):** 배선을 **프로젝트 `.claude/settings.json`(커밋됨)** 에 두고 `.gitignore` 에 예외를 뒀다. 종전에는 전역 설정에만 있어 **컴퓨터를 옮기자 훅이 통째로 사라져 있었고**(이 컴퓨터에 미등록 상태였음) 차단장치가 꺼진 것을 아무도 몰랐다. 또 종전 배선은 `jq -r '.tool_input.code' | node …` 로 **jq+파이프(셸)** 에 의존해 jq 없는 환경에서는 등록해도 안 걸렸다 → `scripts/lib/hook-input.js` 가 훅 JSON 을 직접 파싱해 외부 의존 0. matcher 는 도구 이름 변형(`mcp__figma__*`·`mcp__claude_ai_Figma__*` 등)을 모두 잡도록 `use_figma` 부분매칭.
