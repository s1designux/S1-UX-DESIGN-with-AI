...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-09-15 | **바텀시트 웹 부품화 — 모바일에서 드롭다운 대신 쓸 것이 생겼다.** 정본에 이미 있던 `Bottom Sheet`·`Bottom Sheet Option` 을 배포본으로 옮겼다(새 토큰 0건 · 정본 실재 9칸만, 없는 3칸은 만들지 않음). `date-picker`·`time-picker` 가 자기 시트 CSS 를 버리고 이 부품을 쓰며 **이관 전후 치수 차이 0**. 🤖 component-verifier 5회차 PASS — ⭐ 가 놓친 결함 5건을 검증이 잡았다(스크롤 잠금 영구 잠김 · check 아이콘 굵기 · **안드로이드 누름 표시 상실** 등). river 검수 지적 2건 반영으로 **모바일 마우스오버 없음** 규칙이 Input→Button→Text Button 으로 일관돼졌다. 배포본 0.5.7 → **0.6.10**. | 
