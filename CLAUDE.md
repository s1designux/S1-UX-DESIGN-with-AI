...
> **⛔ 레거시 서비스 패턴은 정본과 분리(river 2026-09-18).** 운영 중인 서비스(모두앱 등)의 화면·패턴은 정본에 합치지 않는다 — 최신 부품을 끼워 넣지도, 그 수치를 정본 근거로 쓰지도 않는다. 역방향도 금지. 보관처 `reports/pattern-builder/profiles/`.
...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-09-18 | **만들어 둔 화면을 빌더로 가져와 이어 만든다(레거시 그대로).** |
