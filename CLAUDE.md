...
**트리거:** `tokens.css`·`site-base.css`·`vars-data.ts`·`semantic.html`·`registry/tokens/semantic.colors.json` 변경 / 항상
...
**정본(Canonical) — 2계층:**
- **Foundation + 레거시 컴포넌트 semantic**(color/button·control·text·icon …) = `vars-data.ts` (FOUNDATION_COLOR + SEMANTIC_COLOR). `tokens:gen`이 tokens.css Semantic을 여기서 생성.
- **역할 기반 semantic**(`--color-bg/surface/text/border/action/status`) = `assets/css/site-base.css` (페이지가 `style.css → @import site-base.css`로 로드하는 라이브 파일). vars-data엔 없음.
...
작업 완료 직전:
  ...
  7. Gate 7 (Token Sync Monitor) — 토큰 값(tokens.css·site-base.css·vars-data·semantic.html·registry) 변경 시 / 항상
```
