...
| 긴 참조 문서에서 **어느 대목인지** 찾기(찾을 낱말을 모를 때) | 🔎 `npm run docs:find -- "<질문>"` → `파일:줄범위` 만 반환(로컬 임베딩, 토큰 0). 낱말을 알면 grep 이 더 싸다. **위치만 주는 도구 — 판정 근거 아님** |
...
| 🖥️ | 로컬 LLM | 이 맥에서 돈 로컬 모델 — **모델명을 반드시 붙인다**(`🖥️ qwen3-embedding:0.6b`). 판정·규칙 저작에는 쓰지 않는다 |
...
**🖥️ 는 별도 목적** — 검증 여부가 아니라 **로컬 모델이 어디에 쓰였는지 활용도를 추적**한다(river 지시 2026-09-04). 로컬 모델이 한 몫은 크든 작든 🖥️ + 모델명으로 표시한다. 🤖(Claude 서브에이전트)와 섞지 않는다.
...
규칙:
- **UI/HTML/CSS를 건드렸으면 크기 불문 렌더 1회 확인.** "검사기·div개수 통과"로 시각 검증을 대체하지 않는다. (`npm run shot -- "pages/…html#섹션" out.png` → 스크린샷 Read 로 육안 대조. **raw chrome 명령 금지** — `--screenshot`/`--dump-dom` 은 파일을 다 쓰고도 종료하지 않아 매번 타임아웃까지 매달린다)
...
- 🚫 **[H6] 정본 거버넌스 — 파생은 증거가 아니고, 정본 신설은 승인 사항이다(2026-08-03).** ①**정본**(`vars-data.ts`·`textstyles-data.ts`·`build-components.ts` — 기계가독 목록 `registry/governance/canon-manifest.json`, **Gate 36**)과 **파생 표면**(`pages/*.html`·`registry/**`·미리보기·`design/DESIGN*.md`·`reports/**`)이 다르면 **저울질 없이 파생을 고친다.** 파생 값은 증거가 아니다. **"파생이 A 인데 정본은 B 다, 어느 쪽이 맞나?"는 성립하지 않는 질문이며 그 형태로 묻는 것 자체가 규칙 위반** — 성립하는 질문은 **"정본을 A 로 바꿀까요?"** 뿐이고, 파생 수정을 비용·부작용처럼 서술하지 않는다. §「두 갈래 분류」는 레거시(DS 2.4)↔정본에만 적용, 정본↔파생에는 금지. ②⭐는 정본에 새 항목(토큰·텍스트 스타일·컴포넌트·variant·**판정 기준·규칙 자체**)을 임의로 만들지 않는다 — **"근거가 없을 때 내가 규칙을 만들어 빈자리를 메우는 것"이 금지의 핵심.** 근거 없으면 `needs-decision`, 사용자 승인 시 추가(**Gate 34 집행** — 승인은 `--approve --by river --reason "..." --quote "<river 실제 발화 그대로>"`. **`--by` 는 자기신고라 믿지 않는다** — 인용문이 세션기록의 사람 발화(`origin.kind=human`)에 실재해야 통과. 2026-09-03 river 지시). 추가 시에도 파생 손편집 금지 — 정본 1곳 수정 후 자동 연동(`tokens:reconcile` 등), 연동이 없으면 연동을 만든다.
...
| 🖥️ | (모델명) | 로컬 모델을 쓴 경우만 — 무엇을 시켰는지 (활용도 추적용) |
...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-09-04 | **Input·Button 파일럿 재승인 — 철회가 승인 범위를 넘었던 것을 바로잡았다.** 2026-09-02 focus-visible 철회가 같은 블록에 있던 **정본 Selected 토큰을 쓰는 초점 표시 규칙까지** 함께 지웠고(최초 커밋부터 있던 것), 그 탓에 오류·정상확인·읽기전용에서 키보드 초점이 사라졌다 — 다크 오류·읽기전용은 초점 전후 계산 스타일 차이가 **0** 이었다. 기계검사 9종이 전부 초록인 채였고 잡은 것은 🤖 `component-verifier` 뿐이다(4회차 만에 PASS, 그 과정에서 내 실수 4건 교정 — 철회 과잉·문서 불일치·마우스 판별 표식이 굳는 경로·**river 가 말하지 않은 읽기전용 예외를 내가 슬쩍 넣은 것**). river 결정 3건 반영: 초점은 정본 Selected 파란 테두리 · Tab 진입 시 커서는 값 끝(마우스·손가락은 누른 자리) · 모바일 지우기 아이콘은 정본 `padR` 12→0 으로 PC 와 같은 자리에. 키보드/포인터 판정은 표식을 **상태로 들고 있지 않고** 직전 포인터 조작 시각을 `event.timeStamp` 로 비교한다(500ms) — 표식이 굳는 경로와 모바일 탭 오판을 구조로 없앤다. river 가 폰(안드로이드·삼성 인터넷)에서 직접 확인해 승인. `reports/repeated-requests.json` 에 `revocation-sweeps-adjacent-legitimate-code` 신설, `self-invented-criteria` +1(→4). **교훈: 되돌릴 때는 '무엇을 승인 없이 만들었나'가 아니라 '이 줄이 어느 토큰을 쓰나'로 가른다.** |
