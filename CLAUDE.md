...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-09-15 | **river 결정 3건 반영 — 달력 주말 색·달력 화살표 부품·모바일 상단바 유형 1종.** 달력에서 토요일은 파랑, 일요일은 빨강으로 구분한다(2026-06-30 '주말 색 미적용' 뒤집음 — 웹은 이미 그렇게 칠하고 있었고 Figma 정본만 전 요일 같은 색이었다). 달력 헤더 화살표를 `Calendar Nav Arrow`(State 3종) 부품으로 빼 손올림·못누름 색을 담을 자리를 만들었다(Gate 34 승인). 모바일 상단바에 `Home / Title + 1 Icon` 을 배포본·안내·검수 화면에 더했다(2026-09-08 승인분이 웹에만 빠져 있었다). 새 토큰 0건 — 만들어져 있던 4개를 연결했다. 배포본 0.5.7 → **0.6.0**. |
