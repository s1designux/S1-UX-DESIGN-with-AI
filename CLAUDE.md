...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-09-15 | **밀도(넓게·보통·좁게) 승격 — 크기를 화면에 한 번만 정한다.** 감싸는 요소에 `data-s1-density` 와 `data-s1-break` 한 번이면 컨트롤 10종이 같은 줄 높이(44·34·28, 모바일 48)로 선다. 새 높이·새 토큰 0건 — 컴포넌트 CSS 의 크기 규칙을 선택자만 바꿔 복사한다. 🤖 component-verifier 시나리오 F 를 **5회** 돌려 누적 결함 8건을 전부 닫았다(고친 것 6건 + river 결정 2건). 모바일에서 드롭다운은 바텀시트로, 멀티 토글은 라디오/체크박스로 대체한다(river 승인) — 독립 바텀시트 부품은 BACKLOG 0-b. 배포본 0.5.3 → **0.5.7**. |
