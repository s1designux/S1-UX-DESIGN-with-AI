...
| **레거시 이름** ↔ 정본 이름 | 🔎 `npm run legacy:resolve -- "<이름>"` — 결정 전이면 스스로 정하지 않는다 (Gate 52) |
...
> **⛔ 레거시 서비스 패턴은 정본과 분리(river 2026-09-18).** 운영 중인 서비스(모두앱 등)의 화면·패턴은 정본에 합치지 않는다 — 최신 부품을 끼워 넣지도, 그 수치를 정본 신설 근거로 쓰지도 않는다. 역방향도 금지. 보관처 `reports/pattern-builder/profiles/`.
...
> **채팅 보고는 전역 규칙이 우선한다 (river 2026-09-16).** river 가 읽는 답변은 전역 CLAUDE.md §1-1~§1-3 을 따른다 — 굵은 결론 한 줄 먼저 · 쉬운 말 · 10줄 안팎 · 과정 서술과 파일 이름·명령어 빼기 · **작업 중 중간 보고 0회**.
> 위 Orchestrator Summary 표는 **기록용**이다. 커밋 메시지·보고 파일에 남기고, 채팅에는 펴지 않는다 — river 가 "자세히" 라고 할 때만 편다. 강제는 전역 보고형식 검문소(훅) — 어기면 보고를 다시 쓰게 막는다.
...
- Orchestrator Summary 를 기록(커밋 메시지·보고 파일)에 남기지 않고 작업 완료를 선언하는 것 — 채팅에는 짧은 보고만 한다
...
| 날짜 | 변경 내용 (한 줄) |
|------|------------------|
| 2026-09-18 | **레거시 서비스 패턴은 정본과 분리해 보관한다(모두앱 등).** |
