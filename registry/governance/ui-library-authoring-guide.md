...
river 검수 전에 오케스트레이터가 정본 전수, PC·Mobile, Light·Dark, 키보드·포커스·ARIA, 다중 인스턴스, 빈 페이지 복붙, 전체·개별 설치 동일성, 사이트·dist 동일성을 실제 배포본으로 검사한다. 정본 충돌, 복합 flow·접근성 동작, 검사 실패·차단처럼 위험도가 높은 변경은 별도 검증자가 같은 범위를 재검증한다.

위험 조건이 없고 실제 렌더·자동 검사·river UX 승인이 모두 완료됐다면 별도 검증을 생략할 수 있다. 이때 생략 결정과 근거를 승격 기록에 남긴다. river는 정본 차이를 찾는 역할이 아니라, 검증을 마친 실제 화면의 UI·UX와 사용 흐름을 승인한다.
