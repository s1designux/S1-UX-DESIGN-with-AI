...
//
// ✅ 파서 계약 해제됨 (2026-08-01 · Phase 1) — 배치 위치·주석 내용은 더 이상 제약이 아니다.
//   종전에는 생성기·검사기 11개가 이 파일을 **소스 텍스트 정규식**으로 긁었기 때문에
//   ① 이 객체가 색 선언부보다 뒤에 있으면 그림자가 색 블록으로 오인되고
//   ② 주석에 "대문자 상수명 + 콜론"을 쓰면 정규식이 거기 먼저 걸려 색 토큰 400여 개가
//      통째로 유실됐다(2026-07-29 실제 발생).
//   지금은 전부 scripts/lib/load-vars-data.js 로 **모듈 로드**한다(esbuild 번들 → require).
//   TS 컴파일러가 보는 값과 동일한 것을 읽으므로 선언 순서·주석·포매팅에 영향받지 않는다.
//   적대적 테스트로 확인(2026-08-01): 이 객체를 파일 끝으로 옮기고 주석에 선언문을 주입해도
//   생성물 byte-diff 0 · Gate 7/7b/29 판정 불변.
//
