...
<!-- S1BottomSheetOption — 승인된 배포본을 그대로 마운트하는 Vue 껍데기.
     MARKUPS 는 dist/examples/bottom-sheet-option*.html 안의 승인된 인스턴스를 그대로 도려낸 것이다. -->
<script setup>
import { onBeforeUnmount, onMounted, ref, watch, watchEffect } from "vue";
// bottom-sheet-option 는 JavaScript 런타임이 없다 — 브라우저 기본 동작만 쓴다.

const MARKUPS = {
  "pc": "<div data-s1-component=\"bottom-sheet-option\" data-type=\"text\" data-state=\"default\">\n    <span data-s1-part=\"label\">항목 1</span>\n  </div>"
};
const DEFAULT_BREAK = "pc";
const BREAKS = ["pc"];
const VARIANTS = ["text","checkbox","radio","list"];
const SIZES = [];
/* variant·size 속성 이름은 컴포넌트마다 다르다 — 승인된 마크업에서 읽어 온 것이다. */
const VARIANT_ATTRIBUTE = "data-type";
...
const props = defineProps({
  variant: { type: String, default: undefined, validator: (value) => ["text","checkbox","radio","list"].includes(value) },
