...
<!-- S1Select — 승인된 배포본을 그대로 마운트하는 Vue 껍데기.
     MARKUPS 는 dist/examples/select*.html 안의 승인된 인스턴스를 그대로 도려낸 것이다. -->
<script setup>
...
function mount() {
  ...
  init(element);
  for (const [eventName, emitName] of [["s1:select:change","change"],["s1:select:open","open"],["s1:select:close","close"]]) {
