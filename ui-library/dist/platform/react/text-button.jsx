...
/* S1TextButton — 승인된 배포본을 그대로 마운트하는 React 껍데기.
   마크업을 새로 쓰지 않는다. MARKUPS 는 dist/examples/text-button*.html 안의 승인된 인스턴스를 그대로 도려낸 것이다.
   host 는 display:contents 라 레이아웃에 끼어들지 않는다. */
import { useEffect, useRef } from "react";
// text-button 는 JavaScript 런타임이 없다 — 브라우저 기본 동작만 쓴다.

export const MARKUPS = {
  "pc": "<button type=\"button\" data-s1-component=\"text-button\" data-variant=\"primary\">\n  <span data-s1-part=\"label\">텍스트버튼</span>\n</button>"
};
...
export default function S1TextButton({ variant, size, breakName = DEFAULT_BREAK, parts, className, style, ...rest }) {
