...
/* S1TextButton — 승인된 배포본 마크업을 그대로 옮긴 React 컴포넌트.
   마크업을 새로 쓰지 않는다. dist/examples/text-button*.html 의 승인 인스턴스를 JSX 로 옮긴 것이다.
   서버에서 그려도 같은 마크업이 나온다. */
import { useId, useRef } from "react";
import { scopeOf, slot } from "./runtime.js";
// text-button 는 JavaScript 런타임이 없다 — 브라우저 기본 동작만 쓴다.
...
function assertAllowed(label, value, allowed) {
  ...
  throw new Error(`[s1-ui] text-button: 승인되지 않은 ${label} "${value}". 쓸 수 있는 값: ${allowed.join(", ")}`);
}

export default function S1TextButton({ variant, parts, className, style, ...rest }) {
  const uid = useId();
  ...
  return (
      <button type="button" data-s1-component="text-button" data-variant={variant ?? "primary"} ref={rootRef} className={className} style={style} {...rest}>
        <span data-s1-part="label" {...(slot(partScope, "label").attrs ?? {})}>
          {slot(partScope, "label").content ?? "텍스트버튼"}
        </span>
      </button>
  );
}
