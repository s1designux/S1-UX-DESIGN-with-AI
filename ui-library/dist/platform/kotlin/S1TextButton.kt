...
/**
 * 승인된 글자 버튼. 누르는 동안에도 색은 그대로고 밑줄만 생긴다(정본 그대로).
...
fun S1TextButton(
    ...
    variant: String = "primary",    enabled: Boolean = true,
    ...
    val key = "$variant|$state"
    val root = S1TextButtonSpec.box(key, "root")
    val label = S1TextButtonSpec.box(key, "label")
    Box(
        modifier = modifier
