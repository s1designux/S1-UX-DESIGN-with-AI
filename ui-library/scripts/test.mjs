...
for (const id of componentIds) {
  ...
  for (const field of ["id", "version", "status", "canonicalFingerprint", "rootSelector", "variants", "sizes", "states", "parts", "dependencies", "icons", "a11y", "jsRequired"]) {
    if (!(field in manifest)) failures.push(`${id} manifest missing ${field}`);
  }
  if (manifest.jsRequired !== false) failures.push(`${id} must remain jsRequired=false`);
  if (id === "button") {
    ...
    const expectedMinWidths = { md: "--sizing-80", xsm: "--sizing-64", xxsm: "--sizing-64", lg: "--sizing-80" };
  ...
  if (module.jsRequired !== false || module.runtime !== null) failures.push(`${id} module unexpectedly requires runtime`);
}
